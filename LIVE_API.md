...
선수의 점수·순위 이력을 세션 단위로 반환합니다. 파라미터가 없으면 가장 최근 세션을 반환하고, `totalSessions`로 조회 가능한 전체 세션 수를 알려줍니다.

### 7.1 세션 판정 규칙

- 최초 관측 또는 직전 세션 종료 후 점수가 바뀐 fetch에서 세션을 시작합니다.
- 마지막 점수 변경 뒤 5회 연속 fetch에서 점수가 같으면 다섯 번째 fetch에서 세션을 종료하며, 이 5개 포인트도 해당 세션에 포함합니다.
- 종료 후 점수가 계속 같으면 새 세션을 만들지 않습니다. 이후 점수가 바뀌거나 순위권에 재진입하면 새 세션을 시작합니다.
- 아직 5회 연속 무변화에 도달하지 않은 최신 세션은 열린 세션으로 보고 최신 fetch까지 반환합니다.
