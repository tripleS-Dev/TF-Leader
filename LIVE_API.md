...
| `GET /api/users/history` | JSON | 정확한 이름에 대응하는 점수·순위 누적 이력 |
...
선수의 모든 저장 스냅샷을 시간순으로 복원하여 점수, 순위, 리그 및 24시간 순위 변동 이력을 반환합니다.
...
이력 조회는 과거 상태 이벤트에서도 이름을 찾습니다. 따라서 과거에 사용한 이름이 선수 식별자와 연결되어 있다면 그 선수의 이후 이력도 함께 복원될 수 있습니다. 반대로 여러 선수가 같은 이름을 사용한 기록이 있으면 한 요청에 여러 선수의 이력이 섞일 수 있으며, 결과는 시각과 순위 순으로 정렬됩니다.
...
| `season` | 문자열 | 조회한 시즌 |
| `count` | 정수 | 반환한 이력 포인트 수 |
| `data` | 객체 배열 | 원본 데이터 갱신 시각 오름차순의 이력 |
...
스냅샷은 있지만 정확히 일치하는 선수를 찾지 못하면 `200 OK`와 빈 배열을 반환합니다.
...
`/api/users/history`에는 페이지네이션이나 개수 제한 파라미터가 없습니다. 일치하는 선수의 저장된 전체 이력을 반환합니다.

## 8. `GET /api/graphs/score.png`

`/api/users/history`와 같은 정확 일치 규칙으로 이력을 찾고 점수 변화를 PNG 그래프로 반환합니다.
